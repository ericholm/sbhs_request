...
class Proxy():
    def __init__(self, *args):
        ...
        if (num_args == 1):
            with open(args[0], "r") as f:
                ...
                self.password = self.normalisePassword(f.readline().strip())

        elif (num_args == 2):
            self.username = self.normaliseUsername(args[0])
            self.password = self.normalisePassword(args[1])
    ...
    @staticmethod
    def normalisePassword(password):
